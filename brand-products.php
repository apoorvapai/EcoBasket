...
<?php 
$bid=$_GET['bid'];
$query=mysqli_query($con,"select brand.id as bid,brand.companyName,brand.companyDescription from brand where id='$bid' ");
while($result=mysqli_fetch_array($query))
{ ?>

                <div class="text-block">
                    <h6 class="display-6 fw-bolder"><?php echo $result['companyName'];?></h6>
                    <p><?php echo $result['companyDescription'];?></p>
                </div>
            <?php } ?>
            </div>
        </header>
...
<?php 
$query=mysqli_query($con,"select products.id as pid,products.productImage1,products.productName,products.productPriceBeforeDiscount,products.productPrice from products where productCompany='$bid' order by pid desc  ");
